...
fn generate_geotable() -> IpLookupTable<Ipv4Addr, GeoLoc> {
    ...
    for line in BufReader::new(f).lines().skip(1) {
        let data = line.unwrap().split(",").map(|x| x.to_string()).collect::<Vec<String>>();
        let geoid : u32 = data[0].parse().unwrap();
        geocode.insert(geoid, GeoLoc {
    ...
    for line in BufReader::new(f).lines().skip(1) {
        ...
        tbl.insert(ip, netmask, geocode.get(&geoid).unwrap_or(&GeoLoc {
            continent: "".to_string(),
            country: "".to_string(),
        }).clone());
    }
        ...
        if let Ok(ip) = network[0].parse::<Ipv4Addr>() {
            for asn in data[1..].iter() {
                if let Some((_,_, loc)) = geo.longest_match(ip) {
                    let entry = result.entry(asn.parse::<u32>().unwrap()).or_insert(HashSet::new());
